...
#include "source/SMBRom.hpp"
#include "source/SMB/SMBEmulator.hpp"
#include "source/SMB/GameManager.hpp"
...
int main(int argc, char** argv) 
{
    ...
    if (argc >= 2) {
        ...
        if (romData) {
            ...
            printf("External ROM failed, using embedded ROM\n");
            romData = const_cast<unsigned char*>(smbRomData);
        }
    } else {
        printf("Using embedded ROM\n");
        romData = const_cast<unsigned char*>(smbRomData);
    }
    
    // Create engine with chosen ROM
    printf("Creating SMBEngine...\n");
    SMBEngine* engine = new SMBEngine(romData);
    ...
    while (running) {
        ...
        if (frameCount % 300 == 0) {
            printf("Running... Frame %d (%s ROM)\n", frameCount, 
                   usingExternalROM ? "External" : "Embedded");
        }
